...
def reclassify_press_releases(session: Session) -> PressReleaseReclassificationResult:
    ...
    Raises:
        classification.FixedCategoryClassificationError: CSV・DB定義が利用できない場合
    """
    ...
    while True:
        batch = release_repository.list_press_release_titles_after_id(
            session, after_id=after_id, limit=_RECLASSIFICATION_BATCH_SIZE,
        )
        if not batch:
            break
        ...
        for release_id, title in batch:
            ...
            values.extend((release_id, category_id) for category_id in category_ids)
        # バッチ内の判定を終えてから、未一致の行も含めて古い結果を取り除く。
        category_repository.delete_press_release_fixed_categories(
            session, tuple(release_id for release_id, _title in batch),
        )
        category_repository.create_press_release_fixed_categories(session, values)
        ...
        if len(batch) < _RECLASSIFICATION_BATCH_SIZE:
            break
        after_id = batch[-1][0]
    return PressReleaseReclassificationResult(processed_count, matched_count, classification_count)
