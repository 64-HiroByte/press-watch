...
from press_watch_api.services.fixed_category_reclassification import PressReleaseReclassificationResult
...
class ReclassifyFixedCategoriesCommandTest(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.reclassify = self.enterContext(patch.object(command, "reclassify_press_releases", return_value=PressReleaseReclassificationResult(3, 2, 3)))
        self.get_factory = self.enterContext(patch.object(command, "get_session_factory", return_value=self.factory))

    def run_command(self, argv: list[str] | None = None, **kwargs) -> int:
        options = dict(session_factory=self.factory, stdout=self.stdout, stderr=self.stderr)
    ...
    def assert_safe_diagnostic(self, diagnostic: str) -> None:
        ...
        for forbidden in ("PRIVATE_", "Traceback", "Exception ignored", "StatementError"):
            self.assertNotIn(forbidden, diagnostic)

    def test_success_commits_once_then_outputs_counts_and_closes(self) -> None:
        operations = Mock()
        for name, method in (("reclassify", self.reclassify), ("commit", self.session.commit), ("close", self.session.close)):
            operations.attach_mock(method, name)
        self.assertEqual(self.run_command(), 0)
        self.session.commit.assert_called_once_with()
        self.assertEqual(json.loads(self.stdout.getvalue()), {"processed_count": 3, "matched_count": 2, "classification_count": 3})
        self.assertEqual(self.stderr.getvalue(), "")
        self.assertEqual(operations.mock_calls, [call.reclassify(self.session), call.commit(), call.close()])
        self.session.rollback.assert_not_called()

    def test_empty_result_is_committed_and_reports_zero_counts(self) -> None:
        self.reclassify.return_value = PressReleaseReclassificationResult(0, 0, 0)
        self.assertEqual(self.run_command(), 0)
        self.assertEqual(self.stdout.getvalue(), '{"processed_count": 0, "matched_count": 0, "classification_count": 0}\n')
        self.session.commit.assert_called_once_with()

    def test_help_and_invalid_arguments_do_not_initialize_database(self) -> None:
        for argv, expected in ((["--help"], 0), (["-h"], 0), (["--help", "--unknown"], 2), (["--unknown", "PRIVATE_ARGUMENT"], 2), (["--he"], 2)):
            with self.subTest(argv=argv):
                output, errors = io.StringIO(), io.StringIO()
                self.assertEqual(self.run_command(argv, stdout=output, stderr=errors), expected)
                if expected == 0:
    ...
    def test_configuration_and_open_failures_do_not_run_service(self) -> None:
        for operation, method in (("configure", self.get_factory), ("open_session", self.factory)):
            with self.subTest(operation=operation):
                method.side_effect = _database_error()
                errors = io.StringIO()
                self.assertEqual(self.run_command(session_factory=None, stderr=errors), 1)
                self.assert_safe_diagnostic(errors.getvalue())
    ...
    def test_service_and_commit_failures_rollback_close_without_output(self) -> None:
        for operation, method in (("reclassify", self.reclassify), ("commit", self.session.commit)):
            for error in (_database_error(), FixedCategoryClassificationError("PRIVATE_RULE_DETAIL"), RuntimeError("PRIVATE_CLASSIFIER_DETAIL"), KeyboardInterrupt()):
                with self.subTest(operation=operation, error=type(error).__name__):
                    self.session.reset_mock()
                    output, errors = io.StringIO(), io.StringIO()
                    method.side_effect = error
                    self.assertEqual(self.run_command(stdout=output, stderr=errors), 1)
                    self.assertEqual(output.getvalue(), "")
                    self.assert_safe_diagnostic(errors.getvalue())
                    self.assertIn(f"operation={operation}", errors.getvalue())
                    self.assertIn("commit_succeeded=false", errors.getvalue())
                    if isinstance(error, KeyboardInterrupt):
                        self.assertIn("reason=operation interrupted", errors.getvalue())
    ...
    def test_output_failure_or_interruption_after_commit_does_not_rollback(self) -> None:
        for method in ("write", "flush"):
            for error in (OSError("PRIVATE_OUTPUT_DETAIL"), KeyboardInterrupt()):
    ...
    def test_close_failure_after_commit_returns_failure(self) -> None:
        ...
        self.assertIn("operation=close commit_succeeded=true", self.stderr.getvalue())
        self.session.rollback.assert_not_called()
    ...
    def test_stderr_failures_do_not_escape_or_prevent_cleanup(self) -> None:
        for method in ("write", "flush"):
            with self.subTest(method=method):
                ...
                errors = Mock()
                getattr(errors, method).side_effect = OSError("PRIVATE_OUTPUT_DETAIL")
                self.assertEqual(self.run_command(stderr=errors), 1)
                self.session.rollback.assert_called_once_with()
                self.session.close.assert_called_once_with()

    def test_help_and_argument_diagnostic_output_failures_return_one(self) -> None:
        for argv in (["--help"], ["--unknown"]):
            for method in ("write", "flush"):
                with self.subTest(argv=argv, method=method):
                    broken = Mock()
                    getattr(broken, method).side_effect = OSError("PRIVATE_OUTPUT_DETAIL")
                    output, errors = (broken, io.StringIO()) if argv == ["--help"] else (io.StringIO(), broken)
                    self.assertEqual(self.run_command(argv, stdout=output, stderr=errors), 1)
                    self.factory.assert_not_called()
...
class ReclassifyFixedCategoriesProcessTest(unittest.TestCase):
    ...
    def test_closed_output_pipes_keep_failure_exit_code_and_hide_details(self) -> None:
        for stream, mode, argv in (("stdout", "success", []), ("stdout", "success", ["--help"]), ("stderr", "failure", []), ("stderr", "success", ["--unknown"])):
            with self.subTest(stream=stream, mode=mode, argv=argv):
                result = self.run_process(stream, mode, argv)
                self.assertEqual(result.returncode, 1)
                for forbidden in (b"PRIVATE_", b"Traceback", b"Exception ignored"):
                    self.assertNotIn(forbidden, (result.stdout or b"") + (result.stderr or b""))
    ...
    def test_closed_standard_descriptors_keep_failure_exit_code(self) -> None:
        for stream, mode, argv in (("stdout", "success", ["--help"]), ("stderr", "failure", [])):
            with self.subTest(stream=stream):
                result = self.run_process(stream, mode, argv, close_descriptor=True)
                self.assertEqual(result.returncode, 1)
                self.assertNotIn(b"Exception ignored", (result.stdout or b"") + (result.stderr or b""))

    def run_process(self, stream: str, mode: str, argv: list[str], *, close_descriptor: bool = False) -> subprocess.CompletedProcess[bytes]:
        script = textwrap.dedent("""
            ...
            failure = StatementError("PRIVATE_DB_DETAIL", "PRIVATE_SQL", {}, RuntimeError("PRIVATE_CAUSE"))
            with (
                patch.object(command, "get_session_factory", return_value=lambda: session),
                patch.object(command, "reclassify_press_releases", return_value=PressReleaseReclassificationResult(3, 2, 3)) as reclassify,
            ):
...
def _database_error() -> StatementError:
    return StatementError("PRIVATE_DB_DETAIL", "PRIVATE_SQL", {"value": "PRIVATE_PARAMETER"}, RuntimeError("PRIVATE_CAUSE"))
