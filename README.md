...
現在は **Phase 2: スクレイピングの最小実装** が完了した段階です。
...
DB 保存、API、フロントエンド画面、定期実行は今後のフェーズで実装します。
...
`--output` は DB 保存前に取得件数、カテゴリ、URL重複、停止理由を確認するための検証用スナップショットです。差分保存、履歴管理、本格的な永続化はまだ行いません。

## Phase 2 実装で考慮した点

- 対象ページの構造、利用条件、robots.txt を確認してから実装する
- 取得対象をタイトル、公開日、詳細ページURL、取得元カテゴリに絞り、DB保存前に検証できる形にする
- 詳細ページURLを重複判定キーとして扱い、将来のDB保存時に一意制約へつなげやすくする
...
次は Phase 3 以降として、取得データをアプリケーションで扱うための土台を進めます。

1. Phase 3: PostgreSQL への接続、テーブル設計、マイグレーション、スクレイピング結果の DB 保存
2. Phase 4: データ取得処理の実行単位、手動実行、初回全件取得、差分取得、ログ方針の整理
3. Phase 5: 一覧取得、検索、カテゴリ絞り込み、ブックマーク操作 API の実装
